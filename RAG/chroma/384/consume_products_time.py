...
def add_data_to_chroma(text: str, id:str):
    ...
    collection.add(
        embeddings=[vector],
        metadatas=[{"products": text}],  # Metadata should be a dictionary
        ids=[id]
    )
...
consumer = KafkaConsumer(
    'kafka_postgres_.webshop.products',
...
def process_messages():
    ...
    for message in consumer:
        try:
            ...
            text = (
                f"Product #{deserialized_message['id']} is named '{deserialized_message['name']}'. "
                f"It belongs to label #{deserialized_message['labelid']} and falls under the category '{deserialized_message['category']}'. "
                f"The product is designed for {deserialized_message['gender']}."
            )
            id = "product_id: " + str(deserialized_message["id"])
