...
def add_data_to_chroma(text: str, id:str):
    ...
    collection.add(
        embeddings=[vector],
        metadatas=[{"order_positions": text}],  # Metadata should be a dictionary
        ids=[id]
    )
...
consumer = KafkaConsumer(
    'kafka_postgres_.webshop.order_positions',
...
def process_messages():
    ...
    for message in consumer:
        try:
            ...
            text = (
                f"Order position #{deserialized_message['id']} belongs to order #{deserialized_message['orderid']}. "
                f"It includes article #{deserialized_message['articleid']} with a quantity of {deserialized_message['amount']} "
                f"at a price of {deserialized_message['price']} each."
            )
            id = "order_position_id: " + str(deserialized_message["id"])
            add_data_to_chroma(text, id)

            record_count += 1
            if record_count == 5985:
...
with open('chroma-384.csv', mode='a', newline='') as file:
    ...
    writer.writerow(["5985", f"{elapsed_time:.2f}"])
