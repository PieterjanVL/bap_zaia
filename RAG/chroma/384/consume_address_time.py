...
def add_data_to_chroma(text: str, id: str):
    ...
    collection.add(
        embeddings=[vector],
        metadatas=[{"address": text}],  # Metadata should be a dictionary
        ids=[id]
    )
...
consumer = KafkaConsumer(
    'kafka_postgres_.webshop.address',
...
# Function to process messages and time it
def process_messages():
    ...
    for message in consumer:
        try:
            ...
            text = (
                f"Address #{deserialized_message['id']} belongs to user #{deserialized_message['customerid']}. "
                f"The address is {deserialized_message['address1']} in the city of {deserialized_message['city']}."
            )
            id = "address_id: " + str(deserialized_message["id"])
...
# Measure the time taken to process messages using timeit
