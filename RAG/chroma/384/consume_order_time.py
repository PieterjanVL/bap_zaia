...
def add_data_to_chroma(text: str, id:str):
    ...
    collection.add(
        embeddings=[vector],
        metadatas=[{"order": text}],  # Metadata should be a dictionary
        ids=[id]
    )
...
def process_messages():
    ...
    for message in consumer:
        try:
            ...
            text = (
                f"Order #{deserialized_message['id']} was placed by customer #{deserialized_message['customer']}. "
                f"The shipping address ID is #{deserialized_message['shippingaddressid']}. "
                f"The total order value is {deserialized_message['total']}, including a shipping cost of {deserialized_message['shippingcost']}."
            )
            id = "order_id: " + str(deserialized_message["id"])
...
with open('chroma-384.csv', mode='a', newline='') as file:
    ...
    writer.writerow(["2000", f"{elapsed_time:.2f}"])
