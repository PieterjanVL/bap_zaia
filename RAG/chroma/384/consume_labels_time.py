...
def add_data_to_chroma(text: str, id:str):
    ...
    collection.add(
        embeddings=[vector],
        metadatas=[{"label": text}],  # Metadata should be a dictionary
        ids=[id]
    )
...
consumer = KafkaConsumer(
    'kafka_postgres_.webshop.labels',
...
def process_messages():
    ...
    for message in consumer:
        try:
            ...
            text = (
                f"Label #{deserialized_message['id']}, named '{deserialized_message['name']}', "
                f"has the slug '{deserialized_message['slugname']}'."
            )
            id = "label_id: " + str(deserialized_message["id"])
            add_data_to_chroma(text, id)

            record_count += 1
            if record_count == 1170:
...
with open('chroma-384.csv', mode='a', newline='') as file:
    ...
    writer.writerow(["1170", f"{elapsed_time:.2f}"])
