...
collection = client.get_or_create_collection(name="chroma_384_webshop", metadata={"hnsw:space": "cosine"})
...
def add_data_to_chroma(text: str, id:str):
    ...
    collection.add(
        embeddings=[vector],
        metadatas=[{"sizes": text}],  # Metadata should be a dictionary
        ids=[id]
    )
...
def process_messages():
    ...
    for message in consumer:
        try:
            ...
            text = (
                f"Size #{deserialized_message['id']} is associated with gender '{deserialized_message['gender']}' "
                f"and falls under the category '{deserialized_message['category']}'. "
                f"The size is #{deserialized_message['size']}. For reference, the US size is '{deserialized_message['size_us']}', "
                f"the UK size is {deserialized_message['size_uk']}, and the EU size is {deserialized_message['size_eu']}."
            )
            id = "size_id: " + str(deserialized_message["id"])
...
with open('chroma-384.csv', mode='a', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["15", f"{elapsed_time:.2f}"])
